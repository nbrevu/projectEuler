...
public class RosalindNkew {
	// ALELUYA, ESTA LIBRERÍA FUNCIONA. Gracias, señor cjb, propietario de https://github.com/cjb/libnewicktree.
	private final static String FILE="F:\\Trabajo\\Programación\\Java\\workspace-euler\\Euler\\src\\com\\rosalind\\InFiles\\rosalind_nkew.txt";
	...
	private static class SingleFound implements Solution	{
		...
		public int distance;
		public SingleFound(TreeNode found)	{
			this.found=found.label;
			distance=0;
		}
		@Override
		public Solution combine(Solution other)	{
	...
	private static class ProblemInput	{
		...
		private Solution solveRecursive(TreeNode node)	{
			...
			if (node1.equals(node.label)||node2.equals(node.label)) solutions.add(new SingleFound(node));
			...
			res.addDistance((int)node.getWeight());
			return res;
		}
	}

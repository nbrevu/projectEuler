...
public class Euler749 {
	...
	private static class PowerCalculator	{
		...
		public PowerCalculator(int maxDigits)	{
			powerCache=createPowerCache(LongMath.pow(10l,maxDigits));
		}
		...
		public long sumValidCases(int[] digits)	{
			...
			for (int i=1;;++i)	{
				...
				result+=sumIfValid(n-1,summary)+sumIfValid(n+1,summary);
			}
		}
		private long sumIfValid(long number,IntIntMap digitsToCompare)	{
			IntIntMap numDigits=extractDigits(number);
			return (numDigits.equals(digitsToCompare))?number:0;
		}
	}
	
	public static void main(String[] args)	{
		long tic=System.nanoTime();
		CombinationIterator iterator=new CombinationIterator(MAX_DIGITS,10,true,false);

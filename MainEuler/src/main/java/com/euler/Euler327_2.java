...
public class Euler327_2 {
	private final static int C=4;
	private final static int R=6;
	private final static int EXPECTED=23;
	...
	private static class InternalState	{
		...
		public String toString()	{
			String[] arr=new String[stored.length];
			for (int i=0;i<arr.length;++i) arr[i]=(i==position)?String.format("{%d}",stored[i]):Integer.toString(stored[i]);
			String extra=(atHand>0)?String.format(" (+%d)",atHand):""; 
			return Arrays.toString(arr)+extra;
		}
	...
	private static class Dispenser implements Operation	{
		...
		@Override
		public boolean canDo(InternalState state) {
			return (state.position<0)&&(howMany<=state.remaining)&&(state.atHand==0);
		}
		@Override
		public InternalState resultAfter(InternalState prev) {
	...
	public static void main(String[] args)	{
		...
		for (List<Operation> s:sequences)	{
			...
			InternalState state=InternalState.initial();
			for (Operation o:s)	{
				state=o.resultAfter(state);
				System.out.println(String.format("\t%s => %s.",o.toString(),state.toString()));
			}
			System.out.println();
		}
	}

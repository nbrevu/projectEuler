...
public class Euler679_2 {
	...
	private static BigInteger[] initForLength3()	{
		BigInteger[] result=new BigInteger[TOTAL_STATES];
		Arrays.fill(result,64,TOTAL_STATES,BigInteger.ZERO);
		for (int i=0;i<64;++i) result[i]=BigInteger.ONE;
		return result;
	}
	
	private static void iterate(BigInteger[] previous,BigInteger[] current,Multimap<Integer,Pair<Integer,Character>> transitionMap)	{
		Arrays.fill(current,BigInteger.ZERO);
		for (Map.Entry<Integer,Pair<Integer,Character>> entry:transitionMap.entries()) current[entry.getKey()]=current[entry.getKey()].add(previous[entry.getValue().first]);
	}
	
	public static void main(String[] args) throws IOException	{
		long tic=System.nanoTime();
		BigInteger[] array1=initForLength3();
		BigInteger[] array2=new BigInteger[TOTAL_STATES];
		...
		for (int i=0;i<SIZE-3;++i)	{
			...
			BigInteger[] swap=array1;
			array1=array2;
			array2=swap;
		}
		BigInteger result=BigInteger.ZERO;
		for (int i:getFinalStates()) result=result.add(array1[i]);
		try	(PrintStream ps=new PrintStream("C:\\out679.txt")){
			ps.println(result);
		}

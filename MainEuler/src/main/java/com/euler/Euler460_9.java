	...
	private static class Solver	{
		...
		public void calculate()	{
			...
			for (int x=0;x<bestSolutions.length;++x)	{
				...
				for (;firstNonNull<bestSolutions[x].length;++firstNonNull) if (bestSolutions[x][firstNonNull]!=null) break;
				for (int y=firstNonNull;y<bestSolutions[x].length;++y) advance(x,y+1,bestSolutions[x][y]);
			}
		}

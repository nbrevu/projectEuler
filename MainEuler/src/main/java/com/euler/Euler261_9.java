...
public class Euler261_9 {
	...
	private static class DiophantineInterface	{
		...
		public boolean addSolutions(long m,LongSet knownKs)	{
			long m_=m+1;
			long m_m=-m*m_;
			long x=LongMath.sqrt(m+1,RoundingMode.UNNECESSARY);
			long i=(x-1)/2;
			...
			if (baseSols.size()!=12) System.out.println("Uy, mira qué guay, para m="+m+" (x="+i+") me salen "+baseSols.size()+" soluciones en vez de 12.");
			else	{
				long k=i*(6+i*(14+8*i));
				long n=i*(3+i*(10+8*i));
				FixedSolution patternSol=new FixedSolution(BigInteger.valueOf(k),BigInteger.valueOf(n));
				if (!baseSols.contains(patternSol)) throw new RuntimeException("Mierda, el patrón falla 😢😢😢.");
			}
	...
	public static void main(String[] args)	{
		...
		for (long x=1;;++x)	{
			long m=4*x*(x+1);
			System.out.println("x="+x+" -> m="+m+"... ("+timeFormatter.format(LocalDateTime.now())+").");
			passedMs.add(m);

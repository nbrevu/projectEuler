...
public class Euler750_2 {
	...
	private static class CardStateManager	{
		...
		public int solve()	{
			for (;;)	{
				...
				if (visited.getOrDefault(state.cards,Integer.MAX_VALUE)<state.currentCost) continue;
				...
				for (int i=0;i<size;++i)	{
					...
					for (int j=0;j<size;++j)	{
						...
						if (p.canConnectTo(q))	{
							...
							int knownCost=visited.getOrDefault(newCards,Integer.MAX_VALUE);
							int newCost=state.currentCost+Math.abs(j-i);
							if (knownCost>=newCost)	{
								visited.put(newCards,newCost);
								states.add(new CardState(newCost,newCards));
							}
						}
					}
				}
	...
	public static void main(String[] args)	{
		// Doesn't finish for N=28. Not workable :(.

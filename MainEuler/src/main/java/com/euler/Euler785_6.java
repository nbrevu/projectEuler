...
public class Euler785_6 {
	...
	public static void main(String[] args)	{
		...
		long sum=0;
		for (int q=3;q<N;q+=2) if (checker.isValid(q))	{
			System.out.println(q+"...");
			List<DiophantineSolution> sols=Diophantine2dSolver.solve(1,0,3,0,-16l*q,-4l*q*q,decomp);
			for (DiophantineSolution d:sols) for (Pair<BigInteger,BigInteger> pair:d)	{
				if ((pair.first.signum()<=0)||(pair.second.signum()<0)) continue;
				long x=pair.second.longValueExact();
				long s=8*q-x;
				long sdelta=pair.first.longValueExact();
				long y=(s-sdelta)/2;
				if (x>y) continue;
				long z=(s+sdelta)/2;

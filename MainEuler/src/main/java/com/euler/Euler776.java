...
public class Euler776 {
	private final static long N=1234567890123456789l;
	private final static int PRECISION=12;
	
	private final static List<Integer> DIGITS=extractDigits(N);
	private final static int ACTUAL_PRECISION=PRECISION+1+(int)Math.ceil(Math.log10(N));
	
	private static List<Integer> extractDigits(long n)	{
	...
	private static class CountAndSum	{
		...
		private CountAndSum addDigit(int digit)	{
			BigInteger newSum=BigInteger.TEN.multiply(sum).add(BigInteger.valueOf(digit).multiply(count));
	...
	public static void main(String[] args)	{
		...
		MathContext context=new MathContext(ACTUAL_PRECISION);
		...
		System.out.println(String.format(Locale.UK,"%.12e",result.doubleValue()));

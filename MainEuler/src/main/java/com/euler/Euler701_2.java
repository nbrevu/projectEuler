...
public class Euler701_2 {
	...
	public static void main(String[] args)	{
		...
		int maxRowIndex=1<<W;
		ObjLongMap<State> currentGeneration=HashObjLongMaps.newMutableMap();
		...
		for (int i=1;i<H;++i)	{
			...
			ObjLongMap<State> nextGeneration=HashObjLongMaps.newMutableMap();
			nextGeneration.ensureCapacity(1000000l);
			for (ObjLongCursor<State> cursor=currentGeneration.cursor();cursor.moveNext();) for (int j=0;j<maxRowIndex;++j) nextGeneration.addValue(cursor.key().connect(j),cursor.value());
			currentGeneration=nextGeneration;
		}
		System.out.println("End: "+currentGeneration.size()+" states.");
		long num=0;
		for (ObjLongCursor<State> cursor=currentGeneration.cursor();cursor.moveNext();) num+=cursor.key().getCount()*cursor.value();
		long den=1l<<(W*H);

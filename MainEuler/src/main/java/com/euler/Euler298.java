...
public class Euler298 {
	...
	private final static int PRECISION_DIGITS=10;
	 ...
	 * any other number appears, it's a miss for both, and the number gets added.
	 */
	private static class IncompleteStatus implements CacheStatus	{
		private final int available;
		public IncompleteStatus(int available)	{
		...
		public boolean equals(Object other)	{
			...
			return available==isOther.available;
		}
		@Override
		public TransitionResult transition(int number) {
			// ACHTUNG! Esto no funciona, necesito que "larry" esté ordenado. Para "larry" no es lo mismo [1, 2, 3] que [3, 2, 1].
			if (number<=available) return new TransitionResult(true,true,this);
			CacheStatus transitionTarget=(available+1<=CACHE_SIZE-1)?INCOMPLETE_STATES.getStatus(available+1):FULL_STATES.getStatus(ARRAY15);
			return new TransitionResult(false,false,transitionTarget);
		}
	}
	
	private static class IncompleteStatusCache	{
		private final IncompleteStatus[] states;
		public IncompleteStatusCache()	{
			states=new IncompleteStatus[CACHE_SIZE-1];
			for (int i=1;i<CACHE_SIZE;++i) states[i-1]=new IncompleteStatus(i);
		}
		public IncompleteStatus getStatus(int in)	{
			return states[in-1];
		}
	}
	...
	private static class FullCacheStatus implements CacheStatus	{
		...
		public FullCacheStatus(int[] larryStatus)	{
			...
			this.hashCode=Arrays.hashCode(larryStatus);
		}
		@Override
		public int hashCode()	{
			return hashCode;
	...
	private static BigDecimal getResult(Map<FullStatus,BigInteger> lastGeneration)	{
		...
		MathContext context=new MathContext(PRECISION_DIGITS);
		return new BigDecimal(numerator).divide(new BigDecimal(denominator),context);
	}
	
	public static void main(String[] args)	{
		long tic=System.nanoTime();
		FullStatus turn1Status=new FullStatus(INCOMPLETE_STATES.getStatus(1),0,0);

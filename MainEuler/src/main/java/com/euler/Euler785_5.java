...
public class Euler785_5 {
	...
	private static class ModularEquationCache	{
		...
		public ModularEquationCache()	{
			...
			solutions.put(2l,HashLongSets.newImmutableSetOf(0l));
			solutions.put(13l,HashLongSets.newImmutableSetOf(9l));
		...
		private static LongSet getSolutions(long prime)	{
			long det=trueMod(prime-12,prime);
			LongSet tonelliShanks=EulerUtils.squareRootModuloPrime(det,prime);
			if (tonelliShanks.isEmpty()) return NO_SOLUTIONS;
			long inv=EulerUtils.modulusInverse(26,prime);
			LongSet result=HashLongSets.newMutableSet();
			for (LongCursor cursor=tonelliShanks.cursor();cursor.moveNext();)	{
				long value=(cursor.elem()-14)*inv;
				result.add(trueMod(value,prime));
			}
			return result;
		}
		...
		private static LongSet henselLifting(LongSet previous,long prime,long powPrev,long pow)	{
			...
			for (LongCursor cursor=previous.cursor();cursor.moveNext();)	{
				long x=cursor.elem();
				long dFx=26*x+14;
				if (dFx%prime==0) manualLifting(x,prime,powPrev,pow,result);
				else	{
		...
		for (int q=3;q<N;q+=add4?4:2,add4=!add4)	{
			...
			for (LongObjCursor<LongIntMap> cursor=decomp.getUnsortedListOfDivisorsWithDecomposition().cursor();cursor.moveNext();)	{
				...
				for (LongCursor solsCursor=sols.cursor();solsCursor.moveNext();)	{
					...
					long qq=26*n+14;
					long rr=13*r;
					if (!EulerUtils.areCoprime(pp,qq,rr)) continue;
					...
					for (LongConvergent conv:Iterables.concat(BASE_SOLUTION,LongFiniteContinuedFraction.getFor(qq,pp+pp)))	{
						...
						long val=pp*conv.p*conv.p-qq*conv.p*conv.q+rr*conv.q*conv.q;
						if (val!=1) continue;
						// ZUTUN!!!!! (-p,-q) should be a solution as well.
						...
						long u=d*x;
						long v=d*y;
						// 7) Reversing unimodular transformation: a=2u+v, b=u+b.
						long a=2*u+v;
						long b=u+v;
						// 8) Final equation solution: X=(32q+a/12), det=b/12. Only valid if integer.
						if ((((32*q+a)%12)==0)&&((b%12)==0))	{
							// Secondary solution!
							long xx=(32*q+a)/12;
							long det=b/12;
							...
							if ((0<=xx)&&(xx<=yy))	{
								...
								if ((zz<=N)&&EulerUtils.areCoprime(xx,yy,zz))	{
									//System.out.println(String.format("ATENCI ÓN, ATENCI ÓN: q=%d, r=%d, d=%d, n=%d, P=%d, Q=%d, R=%d, p=%d, q=%d, \"x\"=%d, \"y\"=%d, u=%d, v=%d, a=%d, b=%d, x=%d, y=%d, z=%d.",q,r,d,n,pp,qq,rr,conv.p,conv.q,x,y,u,v,a,b,xx,yy,zz));

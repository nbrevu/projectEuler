...
public class Euler484_4 {
	private final static long LIMIT=5*LongMath.pow(10l,15);
	
	private final static int MAX_PRIMES_NEEDED=10;	// I believe 8 is actually enough, but the overhead is small, so let's be safe.
	...
	private static class PrimeDataGenerator	{
		...
		public NavigableMap<Long,FullNumberData> getFullPowers()	{
			NavigableMap<Long,FullNumberData> result=new TreeMap<>();
			FullNumberData base=new FullNumberData(new PrimePowerData[0]);
			result.put(1l,base);
			getFullPowersRecursive(0,limit,1l,base,result);
			return result;
		}
		private void getFullPowersRecursive(int currentIndex,long currentLimit,long currentNumber,FullNumberData currentAccumulator,NavigableMap<Long,FullNumberData> result)	{
	...
	private static class ArithmeticDerivativeSummator	{
		private final NavigableMap<Long,FullNumberData> allNumbers;
		...
		public long sum()	{
			long result=0l;
			for (Map.Entry<Long,FullNumberData> entry:allNumbers.descendingMap().entrySet())	{
				FullNumberData data=entry.getValue();

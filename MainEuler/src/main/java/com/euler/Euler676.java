...
public class Euler676 {
	...
	private static class CountAndSum	{
		...
		public void add(CountAndSum other,int times)	{
			BigInteger bigTimes=BigInteger.valueOf(times);
			count=count.add(other.count.multiply(bigTimes));
			sum=sum.add(other.sum.multiply(bigTimes));
		}
		public void addPrefix(long in)	{
			count=count.add(sum.multiply(BigInteger.valueOf(in)));
		}
	}
	...
	private static class FullBaseDefinition	{
		...
		private CountAndSum countAndSum(int[] decomposition,int[] combination,int currentIndex)	{
			if (currentIndex==decomposition.length) return new CountAndSum(1,0);
			CountAndSum base=new CountAndSum();
			/*-
			 * Esto está fatal :O. Arreglar. ZUTUN!
			 */
			for (int i=1+currentIndex;i<decomposition.length;++i)	{
				IntSet toAccumulate=singleSums.get(combination[i]);
				base.add(BigInteger.valueOf(toAccumulate.sizeAsLong()),BigInteger.valueOf(sum(toAccumulate)));
			}
			CountAndSum result=new CountAndSum();
			int limit=decomposition[currentIndex];
			IntSet thisLevel=singleSums.get(combination[currentIndex]);
			int toSumComplete=0;
			for (IntCursor cursor=thisLevel.cursor();cursor.moveNext();) if (cursor.elem()<limit) ++toSumComplete;
			else if (cursor.elem()==limit)	{
				CountAndSum partialResult=countAndSum(decomposition,combination,1+currentIndex);
				partialResult.addPrefix(limit<<((combination.length-currentIndex)*bigBase));
				result.add(partialResult);
			}
			result.add(base,toSumComplete);
			return result;
		}
	}
	...
	public static void main(String[] args)	{
		FullBaseDefinition base=new FullBaseDefinition(3,1);
		CountAndSum result=base.countAndSum(LongMath.pow(10,1));
		// Disasterously wrong :D. ZUTUN! Revise.

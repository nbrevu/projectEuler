...
public class Euler275_8 {
	...
	private static class RowCache	{
		...
		public int countAll(int numThreads) throws InterruptedException	{
			DateTimeFormatter formatter=DateTimeFormatter.ofPattern("HH:mm:ss");
			int result=SPECIAL_CASES;
			class Dispatcher	{
				private int currentValue;
				private int maxValue;
				public Dispatcher(int maxValue)	{
					this.currentValue=0;
					this.maxValue=maxValue;
				}
				public synchronized OptionalInt getNext(int threadId)	{
					if (currentValue>=maxValue) return OptionalInt.empty();
					else	{
						OptionalInt result=OptionalInt.of(currentValue);
						System.out.println("Thread "+threadId+" starts row "+currentValue+" at "+formatter.format(LocalDateTime.now())+"...");
						++currentValue;
						return result;
					}
				}
			...
			class WorkerThread implements Runnable	{
				...
				public WorkerThread(Dispatcher dispatcher,int id)	{
				...
				public void run()	{
					for (;;)	{
						OptionalInt next=dispatcher.getNext(id);
						if (next.isEmpty()) break;
						Row r=validInitialRows.get(next.getAsInt());
						...
						currentSum+=countRecursively(r,fulfilledSets,remaining,counterweight,isSymmetric);
					}
				}
			}
			Dispatcher d=new Dispatcher(validInitialRows.size());
			WorkerThread[] threads=new WorkerThread[numThreads];
			Thread[] actualThreads=new Thread[numThreads];
			for (int i=0;i<threads.length;++i)	{
				threads[i]=new WorkerThread(d,i);
				actualThreads[i]=new Thread(threads[i]);
				actualThreads[i].start();
			}
			for (int i=0;i<threads.length;++i)	{
				actualThreads[i].join();
				result+=threads[i].currentSum;
			}
			return result;
		}
	...
	/*
	 * Shit, it ends in time but it misses some :(.
	 * 13182802
	 * Elapsed 3643.8361739 seconds.
	 */
	public static void main(String[] args) throws InterruptedException	{
		int numThreads=Runtime.getRuntime().availableProcessors()-1;
		long tic=System.nanoTime();
		RowCache rowsCache=new RowCache();
		int result=rowsCache.countAll(numThreads);

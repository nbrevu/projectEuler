...
public class Euler768_4 {
	...
	private static class SortedPartition	{
		...
		public static SortedPartition prepare(RawPartition p)	{
			...
			result.sort(PartitionPiece.DEFAULT_COMPARATOR);
		...
		public boolean isChildOf(SortedPartition other)	{
			...
			return findPathRecursive(origin,target);
		}
		private static boolean findPathRecursive(IntIntMap origin,IntIntMap target)	{
			if (origin.isEmpty()&&target.isEmpty()) return true;
			for (IntIntCursor origCursor=origin.cursor();origCursor.moveNext();)	{
				...
				for (IntIntCursor targCursor=target.cursor();targCursor.moveNext();)	{
					...
					if ((targKey%origKey)==0)	{
						...
						if (origVal>=q)	{
							if (findPathRecursive(remove(origin,origKey,q),remove(target,targKey,1))) return true;
						}
					}
				}
			}
	...
	private static class CompatibilitySummary	{
		...
		private static ObjIntMap<SortedPartition> assignCoefficients(SortedPartition[] allPartitions)	{
			...
			BitSet[] relationships=new BitSet[size];
			for (int i=0;i<size;++i)	{
				relationships[i]=new BitSet(size);
				for (int j=0;j<size;++j) if (i!=j) relationships[i].set(j,allPartitions[j].isChildOf(allPartitions[i])); 
			}
			// DAS DEBUG!!!!!
			for (int i=0;i<size;++i) for (int j=0;j<size;++j) if (relationships[i].get(j)) System.out.println(allPartitions[j]+" is a child of "+allPartitions[i]+"!!!!!");
			// DAS ENDE DES DEBUGS!!!!!
		...
		private static void assignCoefficientsRecursive(int index,BitSet[] relationships,IntIntMap result)	{
			if (result.containsKey(index)) return;
			int value=1;
			for (BitSetCursor cursor=new BitSetCursor(relationships[index],true);cursor.moveNext();)	{
				assignCoefficientsRecursive(cursor.elem(),relationships,result);
				value-=result.get(cursor.elem());
			}

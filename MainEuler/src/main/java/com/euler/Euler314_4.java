...
public class Euler314_4 {
	...
	private static class ShapeFinder	{
		private static class TemporaryInfo	{
			public final int index;
			public final int x;
			public final int y;
			public final double length;
			public final double area;
			public TemporaryInfo(int index,int x,int y,double length,double area)	{
				this.index=index;
				this.x=x;
				this.y=y;
				this.length=length;
		...
		private final TemporaryInfo[] tmpHolder;
		...
		public ShapeFinder(int n,int straightSegmentSize,double maxDeviation)	{
			...
			tmpHolder=new TemporaryInfo[2*n+1];
			tmpHolder[0]=new TemporaryInfo(-1,n,straightSegmentSize,2*straightSegmentSize,n*straightSegmentSize);
			bestSolution=null;
			bestRatio=Double.NEGATIVE_INFINITY;
		}
		private void storeSolution(int currentSize)	{
			TemporaryInfo lastData=tmpHolder[currentSize-1];
			double length=lastData.length;
			double area=lastData.area;
			int curX=lastData.x;
			int curY=lastData.y;
			if (curX==curY+1)	{
				Segment s=DIAGONAL;
			...
			for (int i=currentSize-1;i>=1;--i)	{
				Segment reverse=segments[tmpHolder[i].index];
			...
			if ((bestSolution==null)||(ratio>bestRatio))	{
				...
				for (int i=1;i<currentSize;++i) bestSolution.add(segments[tmpHolder[i].index]);
				bestRatio=ratio;
				// System.out.println(bestRatio+" => "+bestSolution);
			}
		}
		public Pair<List<Segment>,Double> findBestSolution()	{
			findBestSolutionRecursive(1,0);
			return new Pair<>(bestSolution,bestRatio);
		}
		private void findBestSolutionRecursive(int writeIndex,int readIndex)	{
			TemporaryInfo last=tmpHolder[writeIndex-1];
			int x=last.x;
			int y=last.y;
			double cumulLength=last.length;
			double cumulArea=last.area;
			for (int i=readIndex;i<segments.length;++i)	{
				...
				double length=cumulLength+s.length;
				double area=cumulArea+(nextX+(0.5*s.dx))*s.dy;
				tmpHolder[writeIndex]=new TemporaryInfo(i,nextX,nextY,length,area);
				if ((nextX==nextY)||(nextX==nextY+1)) storeSolution(1+writeIndex);
				else findBestSolutionRecursive(1+writeIndex,i);
			}
		}
	}

...
public class Euler492_3 {
	...
	*/
	 ...
	 * ES FUNKTIONIERT!
	 * 
	 * Segundo paso: comprobar que la exponenciación en Z[sqrt(117)] funciona como espero (ejemplo sencillo).
	 * ES FUNKTIONIERT!
	 * 
	 * Tercer paso: comprobar el resultado con aritmética modular para primos pequeños.
	 */
	...
	private static class Number117	{
		...
		public final static Number117 ONE=new Number117(BigInteger.ONE,BigInteger.ZERO);
		...
		public Number117 exp(int pow)	{
			Number117 result=ONE;
			Number117 factor=this;
			while (pow>0)	{
				if ((pow%2)==1) result=result.times(factor);
	...
	public static void main(String[] args)	{
		...
		Number117 test=new Number117(BigInteger.valueOf(37),BigInteger.valueOf(-118));
		Number117 result=test;
		for (int i=2;i<=1087;++i)	{
			result=result.times(test);
			Number117 otherResult=test.exp(i);
			if (!(result.integerPart.equals(otherResult.integerPart)&&result.sqrtPart.equals(otherResult.sqrtPart))) throw new RuntimeException("OH NEIN!!!!!");
		}
		/*

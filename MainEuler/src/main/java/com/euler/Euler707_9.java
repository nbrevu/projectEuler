...
F(199,200)=39798. Diff=229.
		...
		for (int m=2;m<=maxSize;++m) allVals[m]=calculate(m,prime);
		for (int v=1;;++v)	{
			int m=1025*v-1;
			int thisVal=calculate(m,prime);
			int q=m/120;
			int r=m%120;
			int expected=q*allVals[120]+allVals[r];
			String message=String.format("F(%d,%d)=%d. Expected=%d (diff=%d).",m,prime,thisVal,expected,expected-thisVal);

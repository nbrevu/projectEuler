...
public class Euler460 {
	...
	private static class Solver	{
		...
		public Steps getBestSolution()	{
			...
			Steps result=currentGen[0];
			for (int i=1;i<currentGen.length;++i)	{
				Steps candidate=currentGen[i];
	...
	public static void main(String[] args)	{
		...
		Solver solver=new Solver(MAX_HEIGHT,HALF_GOAL);
		Steps bestSolution=solver.getBestSolution();
		double shortestTime=bestSolution.time*2;

...
public class Euler736_4 {
	...
	private static class RPowers	{
		...
		public RPowers(int n)	{
			content=new TreeMap<>();
			content.put(n,n);
			rValue=POWER_OF_2_CACHE.getPowerOf2(n)*n;
		}
		private RPowers(NavigableMap<Integer,Integer> content,long rValue)	{
		...
		public long getSValue()	{
			...
			for (Map.Entry<Integer,Integer> entry:content.entrySet())	{
				int rKey=entry.getKey();
				int rCount=entry.getValue();
				int sKey=cumulativeCount;
				int sCount=rKey-lastRKey;
				result+=sCount*POWER_OF_2_CACHE.getPowerOf2(sKey);
				lastRKey=rKey;
				cumulativeCount+=rCount;
			}
			int sCount=n-lastRKey;
			int sKey=cumulativeCount;
			result+=sCount*POWER_OF_2_CACHE.getPowerOf2(sKey);
			return result;
		}
		public long getFullValue()	{
			return getRValue()-getSValue();
		}
		@Override
		...
		public void getChildren(Set<RPowers> toWrite)	{
			...
			for (int i=0;i<key;++i)	for (int j=1;j<=count;++j)	{
				...
				long diff=j*POWER_OF_2_CACHE.getPowerOf2Diff(key,i);
				toWrite.add(new RPowers(copiedMap,rValue-diff));
			}
		}
		...
		private boolean[] getSequence()	{
			...
			int index=0;
			/*-
			 * boolean[] defaults to false. We can be smart and write the true values only.
			for (int i=n;i>0;--i)	{
				int howManyRs=content.get(i);
				for (int j=0;j<howManyRs;++j)	{
					result[index]=false;
					++index;
				}
				result[index]=true;
	...
	private static RPowers getConfiguration(int initialValue)	{
		for (int n=1+initialValue;;++n)	{
			if (n>10) throw new IllegalStateException("No.");
			...
			while (!cases.isEmpty())	{
				...
				System.out.println("Probando "+currentValue+"...");
				long diff=currentValue.getRValue()-currentValue.getSValue();
	...
	public static void main(String[] args)	{
		for (int i=2;i<=10;++i)	{
			// I'm sure that this is *mostly* right, but there is some error in the iteration scheme, probably in the termination condition.
			RPowers optimalConf=getConfiguration(i);
			long value=optimalConf.evaluateX(i);
			System.out.println(String.format("f(%d)=%d (%s).",i,value,optimalConf.toString()));
		}
	}
}

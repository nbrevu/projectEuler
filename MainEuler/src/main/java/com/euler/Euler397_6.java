...
public class Euler397_6 {
	private final static long K=100;
	private final static long X=500;
	...
	private static class Summaries	{
		...
		public void displayDoubleTriangle(char type,long k,long a,long b,long c)	{
			long commonFactor=EulerUtils.gcd(Math.abs(a),EulerUtils.gcd(Math.abs(b),Math.abs(c)));
			if (commonFactor==1)	{
				System.out.println(String.format("Double triangle (type %c): {%d,%d,%d} (k=%d).",type,a,b,c,k));
			}	else	{
				long ar=a/commonFactor;
				long br=b/commonFactor;
				long cr=c/commonFactor;
				System.out.println(String.format("\tDouble triangle (type %c): {%d,%d,%d} (k=%d). Expected to be a rescaling of {%d,%d,%d}.",type,a,b,c,k,ar,br,cr));
				return;
			}
			long dba=b-a;
			long sba=b+a;

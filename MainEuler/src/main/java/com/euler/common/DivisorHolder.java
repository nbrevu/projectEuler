...
public class DivisorHolder	{
	...
	public void addFactor(long prime,int power)	{
		factors.addValue(prime,power);
	}
	public void removeFactor(long prime)	{
		if (factors.addValue(prime,-1)==0) factors.remove(prime);
	}
	public void removeFactor(long prime,int power)	{
		if (factors.addValue(prime,-power)==0) factors.remove(prime);
	}
	...
	public long getSumOfDivisors()	{
		long prod=1;
		for (LongIntCursor cursor=factors.cursor();cursor.moveNext();) prod*=(LongMath.pow(cursor.key(),cursor.value()+1)-1)/(cursor.key()-1);
		return prod;
	}
	...
	public TwoFactorDecomposer getDecomposer()	{
		return new TwoFactorDecomposer(this);
	}
	...
	public LongObjMap<LongIntMap> getUnsortedListOfDivisorsWithDecomposition()	{
		LongObjMap<LongIntMap> result=HashLongObjMaps.newMutableMap((int)getAmountOfDivisors());
		result.put(1l,HashLongIntMaps.newMutableMap());
		factors.forEach((long prime,int power)->	{
			LongObjMap<LongIntMap> toAdd=HashLongObjMaps.newMutableMap();
			for (LongObjCursor<LongIntMap> cursor=result.cursor();cursor.moveNext();)	{
				long currentFactor=prime;
				for (int i=1;i<=power;++i)	{
					LongIntMap newMap=HashLongIntMaps.newMutableMap(cursor.value());
					newMap.put(prime,i);
					toAdd.put(currentFactor*cursor.key(),newMap);
					currentFactor*=prime;
				}
			}
			result.putAll(toAdd);
	...
	public void powInPlace(int n)	{
		for (LongIntCursor cursor=factors.cursor();cursor.moveNext();) cursor.setValue(n*cursor.value());
	}
	...
	public void addFromFirstPrimes(long n,long[] firstPrimes)	{
		for (;;)	{
			long prime=firstPrimes[(int)n];
			if (prime==0)	{
				addFactor((int)n,1);
	...
	public void combineDestructive(DivisorHolder other)	{
		for (Map.Entry<Long,Integer> entry:other.factors.entrySet()) addFactor(entry.getKey(),entry.getValue());
	}
	public void divideDestructive(DivisorHolder other)	{
		for (Map.Entry<Long,Integer> entry:other.factors.entrySet()) removeFactor(entry.getKey(),entry.getValue());
	}
	public static class PowerCache	{
		private final LongObjMap<IntLongMap> cache;
		public PowerCache(long limit,int[] primes)	{
			cache=HashLongObjMaps.newMutableMap();
			for (int i=0;i<primes.length;++i)	{
				IntLongMap thisPrimePowers=HashIntLongMaps.newMutableMap();
				int power=0;
				long prime=primes[i];
				long value=1;
				do	{
					thisPrimePowers.put(power,value);
					++power;
					value*=prime;
				}	while (value<limit);
				cache.put(prime,thisPrimePowers);
			}
		}
		public long getValue(long[] primes,int[] powers)	{
			long result=1l;
			for (int i=0;i<primes.length;++i) result*=cache.get(primes[i]).get(powers[i]);
			return result;
		}
	}
	public static class TwoFactorDecomposition	{
		public int[] factor1;
		public int[] factor2;
		public TwoFactorDecomposition(int[] factor1,int[] factor2)	{
	...
	public static class TwoFactorDecomposer implements Iterable<TwoFactorDecomposition>,Iterator<TwoFactorDecomposition>	{
		...
		public TwoFactorDecomposer(DivisorHolder source)	{
		...
		public long getFirst(TwoFactorDecomposition decomp,PowerCache powers)	{
			return powers.getValue(factors,decomp.factor1);
		}
		public long getSecond(TwoFactorDecomposition decomp,PowerCache powers)	{
			return powers.getValue(factors,decomp.factor2);
		}
		@Override
		...
		@Override
		public TwoFactorDecomposition next() {
			if (!isCalculated) calculateNext();
			isCalculated=false;
			return new TwoFactorDecomposition(currentStatus,oppositeStatus);
		}
		@Override
		public Iterator<TwoFactorDecomposition> iterator() {
			return this;
		}

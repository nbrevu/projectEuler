...
public class Euler445_3 {
	...
	private static class EfficientDivisorHolder	{
		private final Map<Long,int[]> factors;
		private final long[] firstPrimes;
		public EfficientDivisorHolder(long max)	{
			firstPrimes=Primes.firstPrimeSieve(max);
			factors=new HashMap<>();
		}
		private void addFactor(long prime)	{
			int[] pointer=factors.computeIfAbsent(prime,(Long unused)->new int[1]);
			++pointer[0];
		}
		private void removeFactor(long prime)	{
			int[] pointer=factors.get(prime);
			--pointer[0];
			if (pointer[0]<=0) factors.remove(prime);
		}
		public void add(int in)	{
			for (;;)	{
				long prime=firstPrimes[in];
				if (prime==0)	{
					addFactor(in);
					return;
		...
		public long calculateR(long mod)	{
			long prodBase=1;
			long prodPlus1=1;
			for (Map.Entry<Long,int[]> cursor:factors.entrySet())	{
				long power=EulerUtils.expMod(cursor.getKey(),cursor.getValue()[0],mod);
	...
	public static void main(String[] args)	{
		...
		EfficientDivisorHolder combinatorial=new EfficientDivisorHolder(LIMIT);
		combinatorial.add(LIMIT);
		int interval=1000;
		int nextToShow=interval;
		long result=combinatorial.calculateR(MOD);
		for (int i=2;i<l2;++i)	{
			if (i==nextToShow)	{
				System.out.println(i+"...");
				nextToShow+=interval;
			}
			combinatorial.add(LIMIT+1-i);
			combinatorial.remove(i);
			result+=combinatorial.calculateR(MOD);
		}
		result*=2;	// All but the last numbers must be counted twice.
		combinatorial.add(LIMIT+1-l2);
		combinatorial.remove(l2);
		result+=combinatorial.calculateR(MOD);
		result%=MOD;
		...
		(LIMIT=10^6)
		747100792
		Elapsed 1667.0706074680002 seconds.
		 */
	}
}

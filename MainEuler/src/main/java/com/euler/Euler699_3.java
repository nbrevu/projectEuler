...
public class Euler699_3 {
	...
	private static class DecomposedNumber	{
		...
		public DecomposedNumber(long n,long num,long den,DivisorHolder decomposedNumerator,DivisorHolder decomposedDenominator)	{
			this.n=n;
			this.num=num;
			this.den=den;
	...
	private static DecomposedNumber getFullFactor(long in,Factorer factorer)	{
		...
		long num=sigma/g;
		long den=in/g;
		return new DecomposedNumber(in,num,den,factorer.factor(num),factorer.factor(den));
	}
	...
	private static Multimap<Long,DecomposedNumber> sortByDenominator(Factorer factorer,int maxNumber)	{
		Multimap<Long,DecomposedNumber> result=MultimapBuilder.hashKeys().arrayListValues().build();
		int million=1000000;
		boolean add2=true;
		...
		for (int i=2;i<=maxNumber;i+=add2?2:1,add2=!add2)	{
			...
			if (decomposed.den!=i) result.put(decomposed.den,decomposed);
		}
		return result;
	}
	...
	public static void main(String[] args)	{
		...
		Multimap<Long,DecomposedNumber> precalc=sortByDenominator(factorer,internalLimit);
		System.out.println("Y ahora lo chiquitico.");
		for (long l:MULTIPERFECT_NUMBERS) precalc.put(1l,getFullFactor(l,factorer));
		...
		while (!queue.isEmpty())	{
			...
			for (LongCursor cursor=info.decomposedNumerator.getUnsortedListOfDivisors().cursor();cursor.moveNext();)	{
				Iterable<DecomposedNumber> appended=(cursor.elem()==1)?precalc.get(cursor.elem()):new AppendedCollection<>(precalc.get(cursor.elem()),getFullFactor(cursor.elem(),factorer));

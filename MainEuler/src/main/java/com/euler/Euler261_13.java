...
public class Euler261_13 {
	...
	private static class DiophantineInterface	{
		...
		public boolean getSolutionsWithAlpertron(BigInteger m,LongSet knownKs)	{
			...
			{
				BigInteger minimumK=BigInteger.TWO.multiply(m).multiply(m_);
				if (minimumK.compareTo(bigLimit)>=0) return false;	// Can't have solutions! Don't bother!
			}
			List<DiophantineSolution> solutions=Diophantine2dSolver.solve(m_,BigInteger.ZERO,m.negate(),m_m,m_m,BigInteger.ZERO,decomposer,false);
			...
			System.out.println("Para m="+m+" me salen "+baseSols.size()+" soluciones.");
			BigInteger _2m=m.add(m);
			BigInteger _2m1=_2m.add(BigInteger.ONE);
			BigInteger _2m2=_2m1.add(BigInteger.ONE);
			DiophantineSolutionRecursion baseRecursion=new DiophantineSolutionRecursion(_2m1,_2m,_2m2,_2m1,m,BigInteger.ZERO);
	...
	public static void main(String[] args)	{
		...
		for (SortedSet<Long> passedIs:groupedPassedIs.values()) for (long i:passedIs)	{
			long m=4*i*(i-1);
			if (!passedMs.add(m)) continue;
			if (!solver.getSolutionsWithAlpertron(m,knownKs)) break;
		}
		// Iterate "normal" values of i ("special" values of m) which have two patterns of solutions.
		for (long i=1;;++i) if (ungroupedPassedIs.contains(i)) continue;

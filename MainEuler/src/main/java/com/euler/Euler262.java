...
public class Euler262 {
	...
	public static void main(String[] args)	{
		Point minimumPoint=new Point(MIN_POS,0d);
		PositionFinder finder=new PositionFinder(minimumPoint,EPSILON,COARSE_EPSILON);
		Point corner=new Point(MIN_POS,0d);
		{
			Point initialPoint=new Point(START_POS,START_POS);
			Segment searcher=Segment.between(initialPoint,corner).rescale(0.95);
			Point result=finder.binarySearch(searcher);
			// Ok, this looks good. Next step: look for the point P whose derivative is parallel to the vector from start to P.
			System.out.println(String.format(Locale.UK,"(%.20f,%.20f).",result.x,result.y));
			Point result2=finder.advance(result,0.5,false);
			System.out.println(String.format(Locale.UK,"(%.20f,%.20f).",result2.x,result2.y));	// Too bad :(.
			Point result3=finder.findTangentialPoint(initialPoint,result,0.5,false);
			System.out.println(result3);
		}
		{
			Point initialPoint=new Point(END_POS,END_POS);
			Segment searcher=Segment.between(initialPoint,corner).rescale(0.95);
			Point result=finder.binarySearch(searcher);
			// Ok, this looks good. Next step: look for the point P whose derivative is parallel to the vector from start to P.
			System.out.println(String.format(Locale.UK,"(%.20f,%.20f).",result.x,result.y));
			Point result2=finder.advance(result,0.5,true);
			System.out.println(String.format(Locale.UK,"(%.20f,%.20f).",result2.x,result2.y));	// Too bad :(.
			Point result3=finder.findTangentialPoint(initialPoint,result,0.5,true);
			System.out.println(result3);
		}
		System.out.println();
		Point a=new Point(624.65124786984220000000,48.25310286459188000000);
		Point b=new Point(1536.04242107578720000000,873.03877307400730000000);
		long tic=System.nanoTime();
		int iterations=0;
		for (;a.distance(b)>COARSE_EPSILON;)	{
			a=finder.advance(a,COARSE_EPSILON,false);
			++iterations;
		}

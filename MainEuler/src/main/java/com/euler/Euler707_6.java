...
public class Euler707_6 {
	...
	private final static int BASE=199;
	private final static int MAX_FIBO=199;
	private final static long MOD=1_000_000_007l;
	
	// Found by inspection during previous experiments. Obviously this will be different if BASE is not 199.
	private final static int CYCLE_LENGTH=120;
	
	public static void main(String[] args)	{
		long tic=System.nanoTime();
		BigInteger[] fibos=new BigInteger[MAX_FIBO];
		fibos[0]=BigInteger.ONE;
		fibos[1]=BigInteger.ONE;
		for (int i=2;i<fibos.length;++i) fibos[i]=fibos[i-1].add(fibos[i-2]);
		BigInteger[] baseRanks=new BigInteger[CYCLE_LENGTH];
		baseRanks[0]=BigInteger.ZERO;
		baseRanks[1]=BigInteger.valueOf(calculateSingleRow(BASE));
		for (int i=2;i<CYCLE_LENGTH;++i) baseRanks[i]=BigInteger.valueOf(calculateBlockTridiagonal(i,BASE));
		BigInteger cycleSize=BigInteger.valueOf(calculateBlockTridiagonal(CYCLE_LENGTH,BASE));
		BigInteger modTotient=BigInteger.valueOf(MOD-1);
		BigInteger bigCycle=BigInteger.valueOf(CYCLE_LENGTH);
		BigInteger specialMod=BigInteger.valueOf(1025);
		long result=0l;
		for (int i=0;i<fibos.length;++i)	{
			BigInteger[] division=fibos[i].divideAndRemainder(bigCycle);
			BigInteger q=division[0];
			int rem=division[1].intValueExact();
			BigInteger fullRank=q.multiply(cycleSize).add(baseRanks[rem]);
			BigInteger[] otherDiv=fibos[i].add(BigInteger.ONE).divideAndRemainder(specialMod);
			if (otherDiv[1].signum()==0)	{
				// Special case.
				int lowestBit=otherDiv[0].getLowestSetBit();
				if (lowestBit>3) lowestBit=3;
				fullRank=fullRank.subtract(BigInteger.valueOf(20<<lowestBit));
			}
			long moddedPower=fullRank.remainder(modTotient).longValueExact();
			result+=EulerUtils.expMod(2l,moddedPower,MOD);
		}
		result%=MOD;

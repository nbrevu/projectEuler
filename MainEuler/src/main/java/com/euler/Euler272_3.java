...
public class Euler272_3 {
	...
	// Stupid me! This COUNTS numbers. I need the SUM! Ok, it shouldn't be super difficult.
	private static class SeparatePrimes	{
		...
		private SeparatePrimes(long[] primes3k1,long[] products3k2)	{
			this.primes3k1=primes3k1;
			this.products3k2=products3k2;
		}
		public static SeparatePrimes getFor(int limit3k1,int limit3k2)	{
			...
			return new SeparatePrimes(primes3k1,products3k2);
		}
		...
		private int count3k2Below(long limit)	{
			int search=Arrays.binarySearch(products3k2,limit);
			return (search<0)?(-search-1):(search+1);
		}
		public long countTotal(long limit)	{
			return countNo3(limit)+countOne3(limit)+countMany3(limit);
		}
		private long countNo3(long limit)	{
			return countRecursive(limit,5,0);
		}
		private long countOne3(long limit)	{
			return countRecursive(limit/3,5,0);
		}
		private long countMany3(long limit)	{
			long currentLimit=limit/(3*3);
			long result=0l;
			for (;;)	{
				long currentResult=countRecursive(currentLimit,4,0);
				if (currentResult==0) return result;
				result+=currentResult;
				currentLimit/=3;
			}
		}
		private long countRecursive(long limit,int remainingPrimes,int currentIndex)	{
			if (remainingPrimes<=0) return count3k2Below(limit);
			long result=0l;
			for (int i=currentIndex;i<primes3k1.length;++i)	{
				...
				long subResult=0;
				for (;;)	{
					long currentResult=countRecursive(currentLimit,remainingPrimes-1,i+1);
	...
	public static void main(String[] args)	{
		...
		SeparatePrimes solutionCounter=SeparatePrimes.getFor(LIMIT_3K1,LIMIT_3K2);
		long result=solutionCounter.countTotal(LIMIT);

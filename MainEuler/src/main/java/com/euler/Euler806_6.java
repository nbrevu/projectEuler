...
public class Euler806_6 {
	...
	private static class Solution	{
		...
		public boolean equals(Object other)	{
			...
			Solution s3Other=(Solution)other;
		...
		public SolutionSummary simulate()	{
			...
			for (;;)	{
				...
				if (xorValue==0)	{
					...
					++count;
					sum+=index;
					cases.addValue(new Solution(pegs[0].size(),pegs[1].size(),pegs[2].size()),1l);
				}
	...
	public static void main(String[] args)	{
		for (int i=2;i<=30;i+=2)	{
			...
			boolean parityPreserved=true;
			for (Solution sol:result.cases.keySet())	{
				int m=sol.b;
				if (((m-(i/2))%2)!=0) parityPreserved=false;
			}
			if (parityPreserved) System.out.println("\tParity OK.");
			else System.out.println("PARITY NOT OK, WTF.");
		}
	}
}

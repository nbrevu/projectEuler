...
This problem is the first one where my execution time exceeds half a second ($\approx0.6$s). EDIT: after the Miller-Rabin refactor, reduced to about $0.1$ seconds.
...
This is also the first problem where my algorithm took more than $1$ second. A bit more than $5$ seconds, in fact (EDIT: reduced to less than $0.5$s after refactoring Miller-Rabin). The final solution is composed of these primes: $\llave{13,5197,5701,6733,8389}$.
...
\end{array}
\end{equation*}
Clearly this will be a rational number if an only if $5N^2+2N+1$ is a perfect square. Now here goes a complicated procedure involving a diophantine solution calculation (thanks once again, Alpertron), which yields the following solutions:
\begin{eqnarray*}
g_1&=&2,\\
g_2&=&15,\\
...
\end{equation*}
Since the product of two consecutive numbers can never be a square, the remainder must always be the smallest number. Therefore:
\begin{equation*}
...
I feel like this is one of the problems with the highest Kolmogorov complexity. There is no reliably way to do it magically fast, that I know. The only way to seepd it up a little is checking primality against a small set of primes (I used $<1000$). This really speeds up the computation because it effectively prunes the space without resorting to BigIntegers.

...
It's very simple using inclusion-exclusion. We count how many multiples of $3$, $5$ and $15$ are there, then we use triangular numbers to determine each sum, then we return $\mathrm{sum}\paren3+\mathrm{sum}\paren5-\mathrm{sum}\paren{15}$. So, if the limit is $L=1000$, we define $L_3=\floor{\dfrac L3}$, $L_5=\floor{\dfrac L5}$ and $L_{15}=\floor{\dfrac L{15}}$. The solution is $\dfrac{L_3\paren{L_3+1}}2+\dfrac{L_5\paren{L_5+1}}2-\dfrac{L_{15}\paren{L_{15}+1}}2$.
...
This is a bit tedious, but the culling process is interesting. First, about the sizes of the numbers: the only valid sizes are $\paren2\times\paren3=\paren4$ and $\paren1\times\paren4=\paren4$. Just iterating over these sizes is enough (less than $20000$ products, not bad), but we can cull even more if we take into account that the moduli of the products have to verify $n1+n2+(n1\cdot n2)\equiv0$ (mod $9$). This leaves just a few cases: $\paren{1,4}$, $\paren{3,6}$, $\paren{7,7}$ and $\paren{0,0}$. In the first two cases we also need to consider the reverse pairs, $\paren{4,1}$ and $\paren{6,3}$. So, for each combination of (pair of sizes) and (pair of moduli) we generate the possible pairs using a mere arithmetic progression, and we verify the products. So for each pair or arithmetic progression we have a nested loop, and we can cull a bit further by breaking the inner loop when we hit a product with more than $4$ digits.
...
This problem warms my heart a little because it makes me nostalgic of certain old maths book from my childhood.
...
Oooh! More continued fractions. Project Euler still holds your hand, but it forces you to be careful about precision. Anyway, having the PeriodicContinuedFraction class, this is trivial (although I had to add a new method to check the length of the period).
...
I don't quite agree with the difficulty rating: this is the highest rated problem from the first $100$ ($40\%$ is shared with $90$, but this one has been solved by fewer people), and I don't think it's that complicated, but then again, that might be my years of experience talking.
...
Very stupid, streamlined problem. I hate that the mod is so big that BigInteger is required (it was either that or calculating the power linearly instead of with binary exponentiation). Fortunately, modular binary exponentiation is very fast and BigIntegers won't grow much because $20$ digits is the highest amount needed during the calculations. So this gets solved in about $2$ms.

This is the last problem with $5\%$ difficulty in a while. There are more, but they are very sparsely found.
...
\section{Composites with prime repunit proper-ty} % Fuck the bad box. I want my results to be 0-0-0-nº of pages, no exceptions.
\textbf{Difficulty rating}: 45\%.
...
\begin{itemize}
\item Keep a currentSum value. Initialise at 0. At each step, increase the currentSum with the value we're iterating over. But if the result is negative, set currentSum to 0.
...
Another purely algorithmic problem, superficially similar to the previous one but quite different. We need some level of memoisation-slash-dynamic programming; the brute force algorithm is $O\paren{n^5}$ or at best $O\paren{n^4}$, which is feasible but somewhat prohibitive; but there are ``only'' $O\paren{n^3}$ triangles, so we should be able to reduce the count. There are $N=1000$ levels of elements; at level $i\in\corch{1,N}$ we have exactly $i$ elements, each of which is the apex of $N+1-i$ different triangles; therefore (thanks once again, Faulhaber) we have exactly
\begin{equation*}
...
\end{equation*}
Of course, we use binary exponentiation for that big factor in the middle.
...
\section{Cross-hatched triangles}
\textbf{Difficulty rating}: 70\%.

\textbf{Solution}: 343047. \textit{Solved: Mon, 14 Sep 2015, 05:05}.

\textbf{Math knowledge used}: none.

\textbf{Programming techniques used}: none.

This problem is fucked up in a real sense. The solution can be found with a lot of pencil and paper, and it really is just counting all the different cases. The full explanation can be found at https://www.math.uni-bielefeld.de/~sillke/SEQUENCES/grid-triangles, and there are at least 12 subcases. Finding the formula is grueling, but the formula itself is kind of simple (as expected, only cubic polynomials are needed; and there are $7$ of them in the final formula), so the run time is a bit less $7\mu$s. At this point you might as well finish the job by hand, given that $N=36$ is small.

I guess that you could also let the computer do the job: first you generate all the lines, then you generate all the points, and for each point you generate a list of the points connected to it. Then, for every pair of points in every line, count a triangle for each point that is ``visible'' from both of them, then subtract the colinear ones. It will take more to code and the run time will also be much larger, but in exchange, you retain more of your sanity.

\section{Numbers for which no three consecutive digits have a sum greater than a given value}
\textbf{Difficulty rating}: 45\%.

\textbf{Solution}: 378158756814587. \textit{Solved: Wed, 9 Sep 2015, 05:30}.

\textbf{Math knowledge used}: graph theory.

\textbf{Programming techniques used}: dynamic programming.

Yep, graph theory, albeit in a very simple form. Clearly dynamic programming is the way to go for this problem (although there is a gargantuan recursive formula in the problem thread). But not every state is needed. So I created a list with all the possible states (there are $220$), assigned an index to each of them, and looked for transitions between two consecutive states (i.e.: remove the first digit, add the last one, accept only if the new total sum verifies the conditions); there are $1210$. Now I just need these transitions in order to iterate. I start with an array with all the states for valid three digit combinations (without trailing zeroes!), and then, at each iteration (for $i\in\corch{4,20}$), I loop over the transitions: given the arrays $P$ (previous) and $N$ (next), starting with all zeroes for $N$, for every transition $a\rightarrow b$ I do $N\corch{b}=N\corch{b}+P\corch{a}$.

The majority of the time goes for the fancy transition calculation. Only about $5\%$ goes to the actual update loop, and the total run time is about $66$ms.

\section{Intersections}
\textbf{Difficulty rating}: 65\%.

\textbf{Solution}: 2868868. \textit{Solved: Tue, 15 Sep 2015, 05:41}.

\textbf{Math knowledge used}: algebraic geometry, Euclid's greater common divisor algorithm.

\textbf{Programming techniques used}: custom Rational class.

This is computationally very intensive and I don't think there is much room for improvement. It's just: generate segments, calculate intersections, ensure that there aren't repetitions. I tried to be somewhat clever and grouped segments by slope, but I must have done some ??? ZUTUN! ¿Qué falta aquí D:?

The only ``difficulty'' is in the intersection formula, which is algebraic geometry at high school level. The segments can be codified as $\paren{x,y}+t\paren{\Delta x,\Delta y}$ where $t\in\corch{0,1}$; so, if the other segment is $\paren{x',y'}+u\paren{\Delta x',\Delta y'}$ with $u\in\corch{0,1}$, then the intersection point happens when
\begin{eqnarray*}
t&=&\dfrac{\Delta x'\paren{y'-y}-\Delta y'\paren{x'-x}}{\Delta x'\Delta y-\Delta x\Delta y'},\\
u&=&\dfrac{\Delta x\paren{y'-y}-\Delta y\paren{x'-x}}{\Delta x'\Delta y-\Delta x\Delta y'}.
\end{eqnarray*}
If $t\in\paren{0,1}$ and $u\in\paren{0,1}$, we have found a valid intersection, so we calculate the intersection point using any of the two equations (either with $t$ or $u$). A set must be used in order to prevent duplicates, which do happen.

As I mentioned, this is computationally intensive, so it takes about $4.4$ seconds. There isn't much I can do about that since the segments don't follow any pattern.

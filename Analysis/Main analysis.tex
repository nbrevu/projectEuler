...
This is a reasonably basic approach and it takes less than $4$ms in my machine. However, in the problem thread there are all kinds of wacky super-fast ways to calculate the result, including a lot of combinatorial arcane magic and some unexpected guests like the Catalan numbers. In particular, someone posted (without comment) some code implying that $f\paren n=\displaystyle\binom{2n-1}{n+1}$, which surely sounds kind or right, but it would be nice to prove it (I guess I could do it with a bit of time? I'm very sleepy right now). This reduces the problem to the calculation of a handful of combinatorial numbers (it also implies that $f\paren 1=0$, which again, sounds about right).
...
\textbf{Programming techniques used}: prime power combination generation, lazily evaluated infinite lists.
